...
class PdfViewerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
      ...
      //  setUpToolbar(intent.extras!!.getString(FILE_TITLE, "PDF",))
        ...
        try {
            ...
            try{
                enableDownload = intent.extras!!.getBoolean(ENABLE_FILE_DOWNLOAD, false)
            }catch (e: Exception){
                e.printStackTrace()
            }
        ...
        try {
            isFromAssets = intent.extras!!.getBoolean(FROM_ASSETS, false)
        }catch (e : Exception){
            e.printStackTrace()
        }
    ...
    private fun init() {
        ...
        val title = intent.extras!!.getString(FILE_TITLE)
        binding.tvAppBarTitle.text = title

        binding.pdfView.statusListener = object : PdfRendererView.StatusCallBack {

            override fun onPdfLoadStart() {
                Log.e(TAG,"onPdfLoadStart")
                //runOnUiThread {
            ...
            override fun onPdfLoadProgress(progress: Int, downloadedBytes: Long, totalBytes: Long?) {
                Log.e(TAG,"onPdfLoadProgress : $progress")
                showProgressBar(true)
                //Download is in progress
            }

            override fun onPdfLoadSuccess(absolutePath: String) {
                Log.e(TAG,"onPdfLoadSuccess")
                //runOnUiThread {
  ...
  /*  private fun setUpToolbar(toolbarTitle: String) {
    ...
    private fun initPdfViewer(fileUrl: String?) {
        ...
        try {
            binding.pdfView.initWithUrl(fileUrl!!, headers, lifecycleScope, lifecycle = lifecycle)
        } catch (e: Exception) {
    ...
    private fun clearLastFile(){
        val cachedFileName = getCachedFileName(fileUrl!!)
        val cachedFile = File(cacheDir, cachedFileName)

        if (cachedFile.exists()) {
            Log.e(TAG,"cachedFile exists $cachedFile")
            cachedFile.delete()
        }
    }
    ...
    private fun showProgressBar(result: Boolean) {
        try {
            if (result) {
