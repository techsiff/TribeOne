...
class ManageUsersAdapter(
    private var items: List<GetUsers>,
    private val listener: ManageUserListener
) : RecyclerView.Adapter<ManageUsersAdapter.ViewHolder>() {

    interface ManageUserListener {
        fun onSelectUser(user: GetUsers?)
    }

    @SuppressLint("NotifyDataSetChanged")
    fun updateList(newItems: List<GetUsers>) {
        items = newItems
